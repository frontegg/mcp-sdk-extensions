...
function toBase64url(buffer: ArrayBuffer | Uint8Array): string {
	const buf =
		buffer instanceof ArrayBuffer ? Buffer.from(buffer) : Buffer.from(buffer.buffer, buffer.byteOffset, buffer.byteLength);
...
export class DPoPHandler {
	...
	async init(privateJwk?: JsonWebKey): Promise<void> {
		if (privateJwk) {
			...
			const privateKey = await subtle.importKey('jwk', privateJwk, { name: 'ECDSA', namedCurve: 'P-256' }, true, ['sign']);
			...
			const publicKey = await subtle.importKey('jwk', publicJwk, { name: 'ECDSA', namedCurve: 'P-256' }, true, ['verify']);
			this.keyPair = { privateKey, publicKey } as CryptoKeyPair;
			this.publicJwk = publicJwk;
		} else {
			this.keyPair = (await subtle.generateKey({ name: 'ECDSA', namedCurve: 'P-256' }, true, [
				'sign',
				'verify',
			])) as CryptoKeyPair;
